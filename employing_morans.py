...
from run_gwr_analysis import run_gwr_analysis
...
def find_nearest_geometry(_island, _gdf):
    island_geom = _island.geometry
    other_geometries = _gdf[_gdf.index != _island.name].geometry
    ...
    _nearest_region = _gdf[_gdf.geometry == nearest_geom]
    return _nearest_region


def get_merged_gdf(regions_shapefile_path, real_data):
...
def get_sar_model(variable_names, y, x, w):
    try:
...
def do_model_comparison(sar_model, sdm_model):
    try:
        ...
        aic_sar = n_sar * np.log(rss_sar / n_sar) + 2 * k_sar  # AIC formula for SAR
        ...
        aic_sdm = n_sdm * np.log(rss_sdm / n_sdm) + 2 * k_sdm  # AIC formula for SDM
        ...
        # Add "Intercept" to name_x to match the length of betas
        selected_model.name_x = ["Intercept"] + selected_model.name_x  # Prepend "Intercept"
        return selected_model, model_type  # Return the selected model and its type

    except Exception as e:
        print(f"Error during model comparison: {e}")
        logger.error(f"Error comparing models: {e}")
        raise ValueError("Error in do_model_comparison: ", e)


def get_durbin_model(variable_names, y, x, w):
    try:
...
def create_spatial_weights_matrix(merged_gdf):
...
def plot_residuals(merged_gdf, selected_model, model_type):
    try:
...
def calculate_morans_i_on_residuals(merged_gdf, weights_matrix, model_type):
    try:
        # Calculate Moran's I for Residuals
        residuals = merged_gdf['Residuals'].values
        moran_residuals = esda.Moran(residuals, weights_matrix)
...
def extract_coefficients(selected_model, model_type):
    try:
        # Extract coefficients from the selected model
        # Extract p-values from z_stat
        p_values = [stat[1] for stat in selected_model.z_stat]  # Extract p-values from z_stat

        # Create the DataFrame with the coefficients
        coefficients = pd.DataFrame({
            'Variable': selected_model.name_x,  # Independent variable names
            'Coefficient': selected_model.betas.flatten(),  # Coefficients
            'Std_Error': selected_model.std_err.flatten(),  # Standard errors
            'p_Value': p_values  # Extracted p-values
        })
        ...
        raise ValueError("Error while generating coefficients : ", e)
...
def main():
    ...
    data = {
        ...
        'Natural Vegetation Change (km²) Deforestation': [2411.241831, -61.61709164, 1061.202675, 2341.93647,
                                                          35.6134795,
    ...
    merged_gdf = get_merged_gdf(regions_shapefile_path, real_data)
    ...
    # 3.1. Create Spatial Weights Matrix using Queen contiguity
    weights_matrix = create_spatial_weights_matrix(merged_gdf)
    ...
    moran = esda.Moran(poverty_level, weights_matrix)
    ...
    lisa = esda.Moran_Local(poverty_level, weights_matrix)
    ...
    if merged_gdf['Poverty_Level'].isnull().any():
        ...
        sar_model = get_sar_model(variable_names, y, x, weights_matrix)

        if sar_model is None:
            logger.error("SAR model is None. Halting execution.")
            print("Error: SAR model is None. Halting execution.")
            raise ValueError("SAR model is None.")

        # 4.3. Spatial Durbin Model (SDM)
        sdm_model = get_durbin_model(variable_names, y, x, weights_matrix)

        if sdm_model is None:
            logger.error("SDM model is None. Halting execution.")
            print("Error: SDM model is None. Halting execution.")
            raise ValueError("SDM model is None.")


        # 4.4. Compare Models (Based on AIC)
        selected_model, model_type = do_model_comparison(sar_model, sdm_model)

        if selected_model is None or model_type is None:
            logger.error("No selected model or model type. Halting execution.")
            print("Error: No selected model or model type. Halting execution.")
            raise ValueError("Missing either selected model or model type.")

        print("Selected model : ", selected_model)
        print("Model type : ", model_type)

        # 5. Extract and Display Coefficients
        coefficients = extract_coefficients(selected_model, model_type)

        # Ensure coefficients is not null
        if coefficients is None or coefficients.empty:
            logger.error("No coefficients were extracted. Halting execution.")
            print("Error: No coefficients were extracted. Halting execution.")
            raise ValueError("Coefficients extraction failed. Ensure the model was fitted correctly.")

        # 6. Visualization of Regression Results
        merged_gdf = plot_residuals(merged_gdf, selected_model, model_type)

        # 6.1. Plot Residuals

        # 6.2. Moran’s I on Residuals
        calculate_morans_i_on_residuals(merged_gdf, weights_matrix, model_type)
        ...
        # Run GWR and visualize geographical heterogeneity
        gwr_results = run_gwr_analysis(
            ...
            dependent_var='Poverty_Level'  # Column name for dependent variable
        )
        print('GWR summary')
        logger.info('GWR Summary')
        logger.info(gwr_results.summary)
